...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlacementControllerNetworkObject = null;
		public GameObject[] PlayerManagerNetworkObject = null;
		public GameObject[] PlayerNetworkObject = null;
		public GameObject[] PlayerUiManagerNetworkObject = null;
		public GameObject[] TestNetworkObject = null;
		public GameObject[] UnitNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlacementControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlacementControllerNetworkObject.Length > 0 && PlacementControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlacementControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlacementControllerBehavior>();
						}
					}
			...
			else if (obj is PlayerUiManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerUiManagerNetworkObject.Length > 0 && PlayerUiManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerUiManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerUiManagerBehavior>();
						}
					}
			...
			else if (obj is UnitNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (UnitNetworkObject.Length > 0 && UnitNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(UnitNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<UnitBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlacementController instead, its shorter and easier to type out ;)")]
		public PlacementControllerBehavior InstantiatePlacementControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlacementControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<PlacementControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlacementControllerBehavior>().networkObject = (PlacementControllerNetworkObject)obj;
		...
		[Obsolete("Use InstantiatePlayerUiManager instead, its shorter and easier to type out ;)")]
		public PlayerUiManagerBehavior InstantiatePlayerUiManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerUiManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerUiManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerUiManagerBehavior>().networkObject = (PlayerUiManagerNetworkObject)obj;
		...
		[Obsolete("Use InstantiateUnit instead, its shorter and easier to type out ;)")]
		public UnitBehavior InstantiateUnitNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(UnitNetworkObject[index]);
			var netBehavior = go.GetComponent<UnitBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<UnitBehavior>().networkObject = (UnitNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlacementController
		/// </summary>
		/// <returns>
		/// A local instance of PlacementControllerBehavior
		/// </returns>
		/// <param name="index">The index of the PlacementController prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlacementControllerBehavior InstantiatePlacementController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlacementControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<PlacementControllerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of PlayerUiManager
		/// </summary>
		/// <returns>
		/// A local instance of PlayerUiManagerBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerUiManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerUiManagerBehavior InstantiatePlayerUiManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerUiManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerUiManagerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Unit
		/// </summary>
		/// <returns>
		/// A local instance of UnitBehavior
		/// </returns>
		/// <param name="index">The index of the Unit prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public UnitBehavior InstantiateUnit(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(UnitNetworkObject[index]);
			var netBehavior = go.GetComponent<UnitBehavior>();
